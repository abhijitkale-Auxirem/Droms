...
export default function HabitsPage() {
  ...
  const handleCreate = () => {
    ...
    setShowModal(false);
    setForm({ title: "", frequency: "daily", category: habitCategories[0], icon: "🎯" });
  };
  ...
  const avgCompletion = Math.round(habits.reduce((a, h) => a + h.completionRate, 0) / habits.length);
                ...
                <tr key={habit.id}>
                  ...
                  <td>
                    <div className="flex items-center gap-2">
                      <div className="progress-bar flex-1"><div className={`progress-fill ${getProgressColor(habit.completionRate)}`} style={{ width: `${habit.completionRate}%` }} /></div>
                  ...
                  {habitIcons.map(icon => (
                    <button key={icon} onClick={() => setForm({...form, icon})}
                      className={cn("w-10 h-10 rounded-xl text-xl flex items-center justify-center transition-all",
                        form.icon === icon ? "bg-primary-100 border-2 border-primary-500" : "bg-slate-100 hover:bg-slate-200"
                      )}>{icon}</button>
                  ))}
                </div>
              </div>
